...
function loadUserInfo() {
    ...
    if (user) {
        ...
        if (userElement) {
            userElement.textContent = user.nome || user.usuario;
        }
    }
}
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Sistema de login (só na página de login)
...
function inicializarSistemaEquipamentos() {
    ...
    if (btnFechar) {
        btnFechar.addEventListener('click', fecharModalEquipamento);
    }
    
    const btnCancelar = document.getElementById('btn-cancelar-equipamento');
    if (btnCancelar) {
        btnCancelar.addEventListener('click', fecharModalEquipamento);
    }
...
function salvarEquipamento(e) {
    ...
    if (document.getElementById('total-equipamentos')) {
        atualizarDashboard();
    }
}
